...
import { MockUser, JWTPayload, MockJWTResult } from '../types';
...
export const createMockJWT = (
  userPayload: Partial<JWTPayload> & { email: string }
): MockJWTResult => {
  ...
  const refreshToken = `ref_${base64UrlEncode(userPayload.email)}_${
    now + 7 * ONE_DAY_MS
  }`;
 ...
 */
export const decodeMockJWT = (token: string | null | undefined): JWTPayload | null => {
